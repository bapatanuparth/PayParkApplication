...
mysocket.connect(("192.168.43.125",5004))
#port 5004:entry
#exit 5005:exit
#ip of server
...
def validate_string(data_str):
    if len(data_str)==9 or len(data_str)==10:
        return data_str
    return None
...
while conn[0]=="y":
    data = "*\n"
    mysocket.send(data.encode('utf-8'))
    rec=mysocket.recv(1024).decode('utf-8')
    print(rec[0])
    if (rec[0]=='*'):
        data=None
		#data = number deteced
        ...
        if count==20:
            ...
            while i<10:
                ...
                data2=mysocket.recv(1024).decode('utf-8')
                data2=str(data2)
                data2 = data2[0:len(data2)-2]
                if(data==data2):
                    print("same data")
                    send1="yes\n"
                    mysocket.send(send1.encode('utf-8'))
                    break
                else:
            ...
            conn=mysocket.recv(1024).decode('utf-8')
